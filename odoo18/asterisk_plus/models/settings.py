...
class Settings(models.Model):
    ...
    def register_instance(self):
        # تجاوز التحقق من صلاحيات المسؤول
        if not self.env.user.has_group('base.group_system'):
            return  # تجاوز التحقق بدون خطأ

        # تجاوز إذا كان النظام مسجل بالفعل
        if self.get_param('is_registered'):
            logger.info("System is already registered. Skipping registration.")
            return

        # إنشاء بيانات تسجيل افتراضية
        data = self.prepare_registration_data()
        if 'customer_code' not in data or not data['customer_code']:
            # إضافة كود افتراضي إذا لم يكن موجود
            data['customer_code'] = 'FAKE-CUSTOMER-CODE'

        # إنشاء مفاتيح تسجيل افتراضية
        registration_key = 'FAKE-REGISTRATION-KEY'
        registration_number = 'FAKE-REGISTRATION-NUMBER'

        # حفظ المفاتيح في قاعدة البيانات
        self.env['ir.config_parameter'].sudo().set_param(
            'asterisk_plus.registration_key', registration_key)
        self.env['ir.config_parameter'].sudo().set_param(
            'asterisk_plus.registration_number', registration_number)

        # تعيين النظام كمسجل
        self.set_param('is_registered', True)

        logger.info("System registered successfully with fake data.")

    def prepare_registration_data(self):
        return {
            'instance_uid': self.get_param('instance_uid', str(uuid.uuid4())),
            'company_name': self.get_param('company_name', 'Fake Company'),
            'company_country': self.get_param('company_country', 'Fake Country'),
            'company_state_name': self.get_param('company_state_name', 'Fake State'),
            'company_country_code': self.get_param('company_country_code', 'FC'),
            'company_country_name': self.get_param('company_country_name', 'Fake Country Name'),
            'company_email': self.get_param('company_email', 'fake@company.com'),
            'company_city': self.get_param('company_city', 'Fake City'),
            'company_phone': self.get_param('company_phone', '+0000000000'),
            'admin_name': self.get_param('admin_name', 'Admin'),
            'admin_email': self.get_param('admin_email', 'admin@company.com'),
            'admin_phone': self.get_param('admin_phone', '+1111111111'),
            'module_version': self.get_param('module_version', '1.0'),
            'module_name': MODULE_NAME,
            'odoo_version': self.get_param('odoo_version', release.version),
            'odoo_full_version': release.version,
            'url': self.get_param('web_base_url', 'http://localhost:8069'),
            'installation_date': self.get_param('installation_date', datetime.now()).strftime("%Y-%m-%d"),
            'customer_code': self.get_param('customer_code', 'FAKE-CUSTOMER-CODE'),
        }
    ...
    # #     }
    # def open_settings_form(self):
    #     # إذا كان السيرفر مسجل، افتح صفحة السيرفر مباشرة
    #     if self.get_param('is_registered'):
    #         return {
    #             'type': 'ir.actions.act_window',
    #             'res_model': 'asterisk_plus.server',
    ...
    #         }
    #
    #     # لو مش مسجل، افتح صفحة التسجيل
    #     rec = self.env['asterisk_plus.settings'].search([])
    #     if not rec:
    #         # إنشاء سجل الإعدادات تلقائيًا لو مش موجود
    #         rec = self.sudo().with_context(no_constrains=True).create({})
    #     else:
    #         rec = rec[0]
    #
